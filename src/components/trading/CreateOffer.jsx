...
import { avocado_to_slice, colouredcoin_to_slice } from '../../util/avocado';
  ...
  function handleAdd() {
    ...
    const slice =
      wallets[wallet_id.value].type === COLOURED_COIN
        ? colouredcoin_to_slice(amount_input.value)
        : avocado_to_slice(amount_input.value);

    const trade =
      buy_or_sell.value === 1
        ? newBuy(slice, wallet_id.value)
        : newSell(slice, wallet_id.value);

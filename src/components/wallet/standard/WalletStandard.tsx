  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of avocado in the blockchain at the current
...
function SendCard(props: SendCardProps) {
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      dispatch(
        openDialog(
          <AlertDialog>
            <Trans>
              Error: Cannot send avocado to coloured address. Please enter a avocado
              address.
            </Trans>
          </AlertDialog>,
        ),
    ...
    if (address.slice(0, 12) === 'avocado_addr://') {
    ...
    const amountValue = Number.parseFloat(avocado_to_slice(amount));
    const feeValue = Number.parseFloat(avocado_to_slice(fee));

...
global.daemon_rpc_ws = `wss://${self_hostname}:54677`;
...
function loadConfig(net) {
  try {
    // check if AVOCADO_ROOT is set. it overrides 'net'
    const config_root_dir =
      'AVOCADO_ROOT' in process.env
        ? process.env.AVOCADO_ROOT
        : path.join(os.homedir(), '.avocado', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 54677); // jshint ignore:line

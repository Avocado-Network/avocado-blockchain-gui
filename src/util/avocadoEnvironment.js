...
const PY_FOLDER = '../avocado/daemon';
...
const getAvocadoVersion = () => {
  let version = null;
  const exePath = getExecutablePath('avocado');
  // first see if we can get a avocado exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or avocado is on the path
    try {
      version = child_process
